...
def infobox_hero(hero_data):
    ...
    for parameter in parameter_order:
        for key, value in hero_data.items():
            ...
            if (
                SpiritScaling_present
                and any(key.endswith(scalingStat) for scalingStat in hero_data['SpiritScaling'])
                and key != 'RoundsPerSecond'
            ):  # if it has nonzero spirit scaling
            ...
            elif LevelScaling_present and any(
                key.endswith(scalingStat) for scalingStat in hero_data['LevelScaling']
            ):  # if it has nonzero level scaling
