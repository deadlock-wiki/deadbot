...
cp "$DEADLOCK_PATH/game/citadel/steam.inf" "decompiled-data/version.txt"
 ...
 # Detect the operating system
  if [[ "$OSTYPE" == "darwin"* ]]; then
      # MacOS
      DECOMPILER_EXEC="Decompiler"  # Mac executable without .exe
  elif [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]]; then
      # Windows (via Git Bash or Cygwin)
      DECOMPILER_EXEC="Decompiler.exe"  # Windows executable with .exe
  else
      echo "Unsupported operating system."
      exit 1
  fi

# Loop through files and run Decompiler for each
for FILE in "${FILES[@]}"; do
  INPUT_PATH="$DEADLOCK_PATH/game/citadel/pak01_dir.vpk"
  VPK_FILEPATH="${FILE}.vdata_c"

# Run the decompiler
"$DECOMPILER_PATH/$DECOMPILER_EXEC" -i "$INPUT_PATH" --output "$OUTPUT_DIR" --vpk_filepath "$VPK_FILEPATH" -d
